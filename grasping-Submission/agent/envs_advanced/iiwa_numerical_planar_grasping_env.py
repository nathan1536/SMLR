...
class IiwaNumericalPlanarGraspingEnv(IiwaSampleEnv):
    def __init__(self, max_ts, orientation_control, use_ik, ik_by_sns, state_type, enable_render=False, task_monitor=False,
                 ...
                 robotic_tool="_gripper", backbone='vgg16', use_mlp=True, env_id=0):
        ...
        #########################################
        if np.isclose(np.asarray([self.reward_pose_weight]), np.asarray([0]), atol = 0.5): # No rotation control - 2DoF
            ##################
            # Viewer-related #
            ##################
            # Important while using image base grasping(r3m or vgg with mlp, only 2dof is allowed, no rotation part
            # is included) 3DoF control is not usable with image based grasping right now, as the MLP only predicts
            # 9 values, out of which 7 are joints and delta x and delta z)
            ...
            # now we use latent vector from r3m instad of joint angles and error difference in x and y for 2 dof and also rot error for 3 dof
            if self.use_mlp == True:
                self.observation_indices['obs_len'] = 9

            else:
                self.observation_indices['obs_len'] = 2048
            ...
            if self.use_mlp == True:
                ...
                self.observation_space_dimension = 2048
        ...
        # TASK_NOTE: AUTOMATIC normalization in PPO LOOK UP 
    ...
    # def mlp_accuracy(self, grd_truth, mlp_output):
    #     temp = np.zeros(9)
    #     temp[:2] = mlp_output[7:]
    #     temp[2:] = mlp_output[:7]
    #
    #     err = (abs(grd_truth - temp) / (abs(grd_truth) + 1e-10)) * 100
    #     err = np.reshape(err, (9, 1))
    #     all = np.array([[100], [100], [100], [100], [100], [100], [100], [100], [100]])
    #     #print(temp, '\n', grd_truth , '\n', '\n', '\n')
    #     # print(f"temp:{temp}", '\n', f"grd_truth:{grd_truth}")
    #     acc_vector = all - err
    ...
    def update(self, observation, time_step_update=True):
        ...
        # self.mlp_obs = observation['mlp_observation']
        # self.mlp_obs = mlp_observation

        # normalized outputs as state
        # mlp_joint_obs = self.mlp_obs[:7]
        # print(f":7: {self.mlp_obs[:7]}")
        # self.mlp_obs[:7] = self.normalize_joints(mlp_joint_obs)
        # self.mlp_obs[7] = self.mlp_obs[7] / 1.4
        # self.mlp_obs[8] = self.mlp_obs[7] / 0.73
        # x_err / 1.4, z_err / 0.73
        # print(self.mlp_obs)
        ...
        #state = self.mlp_obs
        # acc = self.mlp_accuracy(groundtruth_state, state) # (9,1) tensor
        # acc_folder = 'mlp_acc/'
        # writer = SummaryWriter(log_dir=acc_folder)
    ...
    def reset(self, temp=False):
        ...
        # if mode == "eval":
        #     object_X, object_Y, object_Z, object_RX, object_RY, object_RZ = list_of_20_box_positions[self.id]
        # print(f"env_id:{self.id}")
